...
XmlRpcRequest.prototype.send = function(callback) {
    ...
    var dom = Builder.buildDOM(xml_call);
    if (async) {
        var that = this;
        xhr.onreadystatechange = function() {
            if (xhr.readyState != 4)
                return;
            if (xhr.status == 200)
                callback(null, new XmlRpcResponse(xhr.responseXML));
            else
                callback("XMLHttpRequest error: '" + that.serviceUrl +
...
XmlRpcResponse.prototype.unmarshal = function(node, parent) {
    ...
    if ((node.nodeType == 3) && (/[^\t\n\r ]/.test(node.nodeValue))) {
        if (this.currentIsName == true) {
            ...
            case "i4":
            case "int":
                this.params[this.params.length - 1] = parseInt(node.nodeValue, 10);
                break;
            case "double":
                this.params[this.params.length - 1] = parseFloat(node.nodeValue);
                break;
            case "string":
                this.params[this.params.length - 1] = node.nodeValue;
                break;
            case "base64":
